		...
		bool		aDynamic)
{
		...
		dropboxShape.SetAsBox(aW/2, aH/2);
	   	...
	   	myCollisionBody->CreateFixture(&dropboxShape, 0.05f)->SetUserData((UserData*)this);
	...
	float 				aYTranslate)
{
	...
	mySprite.DrawTiling(
				aTarget,
				(myX-aXTranslate)*aXScale,
				(myY-aYTranslate)*aYScale,
				(myX+myW-aXTranslate)*aXScale,
		...
		const b2Manifold* 	oldManifold)
{
	if (myIsCloud)
	{
		...
		if (myCollisionBody->GetPosition().y < otherFix->GetAABB().upperBound.y)
